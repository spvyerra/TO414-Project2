...
shark_train <- shark_rand[-test_set, ]
shark_test <- shark_rand[test_set, ]
...
```{r}
predictor_vars <- shark_norm

sqrt(nrow(shark_rand))

predicted_knn <- knn(train = shark_norm, test = shark_norm,
                      cl = as.factor(shark_norm$Deal), k=21)
summary(predicted_knn)

predicted_knn <- as.factor(predicted_knn)

CrossTable(x = predicted_knn, y = shark_norm$Deal, prop.chisq = FALSE)
confusionMatrix(as.factor(shark_norm$Deal), predicted_knn, positive = "1")
```
